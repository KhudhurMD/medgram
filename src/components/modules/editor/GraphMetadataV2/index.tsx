...
import { graphMetadataSlice } from '../GraphMetadata/slice';
...
export default function GraphMetadataV2() {
  ...
  return (
    <div className="absolute w-full">
      <div className="z-10 px-3 py-1.5 rounded-xl fixed sm:absolute border border-gray-200 sm:top-3 top-14 w-fit transform bg-white/70 backdrop-blur-sm right-[50%] translate-x-1/2">
          ...
          {({ open, close }) => (
            <>
              <div className="flex flex-row justify-between ">
                ...
                flex flex-row items-center rounded-md px-1 md:px-2 text-[17px] py-1 text-lg text-primary-800 text-opacity-90 outline-none hover:text-opacity-100 whitespace-nowrap`}
                  >
                    <span>{graphMetadata.name.length > 20 ? `${graphMetadata.name.slice(0, 20)}...` : graphMetadata.name}</span>
                    <CaretDown
                      weight="bold"
